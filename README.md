...
한국 주식 자동매매 시스템. LightGBM 기반 5영업일 수익률 예측 + KIS Open Trading API 연동.
...
매일 자동으로:
1. **데이터 수집** — 약 3000 종목의 일별 OHLCV + 외국인/기관 수급 + 미국 거시지표
2. **지표 계산** — 24개 피처 (이동평균·RSI·MACD·볼린저·변동성·거시·수급 등)
3. **모델 예측** — 섹터별 LightGBM 모델이 "5영업일 후 +2% 이상 오를 확률" 계산
4. **자동 매매** — 상위 3종목 매수 + 변동성 기반 사이징
5. **리스크 관리** — 손절 -5% / 익절 +10% / 5영업일 시간 청산 / 일일 손실 -2% 도달 시 중단
...
│  ├─ supply_demand.py         외국인/기관 매매 (KIS API 우회)   │
...
│  └─ kis_api.py               KIS API 래퍼 (토큰 캐시 + 호출)   │
└────────────────────────────────────────────────────────────────┘
                  ↓
┌────────────────────────────────────────────────────────────────┐
│  검증 (trader)                                                  │
...
python -m src.collector.supply_demand        # 수급 (최근 30일)
python -m src.processor.indicators all       # 24개 피처 생성
...
| 매일 06:50 | 주도 섹터 스캔 |
| 매일 09:00 | 장중 모니터 + 시장 스캐너 가동 |
| 매일 10:05 | 한국 자동 매매 |
| 매일 22:20 | 미국 스캔 (선택) |
| 매일 22:30 | 미국 자동 매매 (선택) |
...
| 항목 | 기본값 | 설정 위치 |
|------|--------|----------|
| 매수 임계값 (prob_threshold) | 0.55 | `config.yaml` |
| 보유 기간 (holding_days) | 5 영업일 | `config.yaml` |
| 손절 (stop_loss) | -5% | `config.yaml` |
...
| 동시 보유 종목 수 | 최대 3개 (Top 매수 신호) | `config.yaml` (top_n) |
...
### 4. 무료 API 의 한계
- KIS API: 외국인/기관 매매는 **최근 30일만** 제공 → 9년 백필 불가
- 호출 빈도 제한 (모의 ~2 req/s)
...
### 8. 자본 분산 부족
- 동시 보유 종목 최대 3개 → 분산 효과 제한
- 한 종목 -10% 손실이 전체 자본 -3.3% 영향
